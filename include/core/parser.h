...
namespace json {
	...
	struct parse_result {
		...
		std::vector<std::unique_ptr<error>> errors; // ошибки полученные при парсинге
	};
	...
	namespace core {
		...
		namespace impl {
			...
			private:
				...
				using err_vect = std::vector<std::unique_ptr<error>>;
