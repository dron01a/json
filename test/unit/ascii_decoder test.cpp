...
TEST_CASE(ascii_decoder_next_char_test) {
	input _input(std::make_unique<string_input_policy>("data€€"));
	ascii_decoder _decoder(_input);
	...
	TEST_ASSERT(_read_res[5] == '?');
}

TEST_CASE(ascii_decoder_current_char_test) {
	input _input(std::make_unique<string_input_policy>("data€€"));
	ascii_decoder _decoder(_input);
	char32_t _c = _decoder.next_char();
	while (_c != eof_char()) {
...
TEST_CASE(ascii_decoder_peek_char_test) {
	input _input(std::make_unique<string_input_policy>("data€€"));
	ascii_decoder _decoder(_input);
	char32_t _c = _decoder.next_char();
...
TEST_CASE(ascii_decoder_push_buff_test) {
	input _input(std::make_unique<string_input_policy>("data€€"));
	ascii_decoder _decoder(_input);
