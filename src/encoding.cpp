	...
	while (_c != '\"') {
		...
		case '\\':
			...
			case 't':
				result += '\t'; break;
			case 'u':
				result += read_unicode(_src); break;
			default:
				throw; // Œÿ»¡ ¿ 
			}
			break;
...
std::string json::encodings::utf8::decoder::read_unicode(json::io_base::i_input * _src){
	std::string result;
	uint32_t code = 0; 
	...
	if (code >= 0xD800 && code <= 0xDBFF) { // ÔÓ‚ÂˇÂÏ Ì‡ Ì‡ÎË˜ËÂ ÒÛÓ„‡ÚÌÓÈ Ô‡˚
		uint32_t low_code = 0;
		for (size_t i = 0; i < 4; ++i) {
			char c = _src->next_char();
			if (c == '\0') {
...
std::string json::encodings::ascii::decoder::decode(json::io_base::i_input * _src) {
	std::string result;
	result.reserve(64);
	char _c = _src->next_char();
	while (_c != '\"') {
